...
class Wireless {
    ...
    static connect(ssid, password, iface) {
        command.exec("ip", "addr", "flush", "dev", `'${iface.replace(/'/gi, "'\"'\"'")}'`);
        ...
        flags.push("wifi");
        flags.push("connect");
        flags.push(`'${ssid.replace(/'/gi, "'\"'\"'")}'`);

        if (password && password !== "") {
            flags.push("password");
            flags.push(`'${password.replace(/'/gi, "'\"'\"'")}'`);
        }
        ...
        command.exec("nmcli", ...flags);
        command.exec("systemctl", "restart", "NetworkManager");
    }
